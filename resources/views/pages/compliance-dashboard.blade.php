        ...
        <div class="max-w-7xl mx-auto header-content">
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-8">
                ...
                <div class="text-right hidden md:block border-l-2 border-brand-green pl-6">
                    ...
                    <p class="text-white text-xl mono-text font-bold">{{ date('H:i') }} <span class="text-brand-green">EAT</span></p>
                    <p class="text-blue-200 text-sm opacity-80">{{ date('d M, Y') }}</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <div class="max-w-7xl mx-auto px-6 -mt-20 relative z-30">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            ...
            <div class="metric-card p-8">
                ...
                <p class="text-gray-500 text-[10px] font-black uppercase mb-1">{{ $m[0] }}</p>
                <h3 class="text-4xl font-extrabold text-brand-blue tracking-tight">{{ $m[1] }}</h3>
                <p class="text-gray-400 text-xs mt-4 font-medium">{{ $m[3] }}</p>
            </div>
            @endforeach
        </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="audit-row p-6 rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-6">
                        <div class="flex items-center gap-4">
                            <i class="fas fa-check-circle {{ $audit[1] == '100%' ? 'text-brand-green' : 'text-blue-300' }} text-xl"></i>
                            <span class="font-bold text-brand-blue uppercase text-sm tracking-tight">{{ $audit[0] }}</span>
                        </div>
                        <div class="flex items-center gap-6">
                            <div class="text-right">
                                <p class="mono-text text-sm font-black text-brand-blue">{{ $audit[1] }}</p>
                                <p class="text-[9px] uppercase font-bold text-gray-400 tracking-widest">{{ $audit[2] }}</p>
                            </div>
                            <div class="h-10 w-1 {{ $audit[3] }} rounded-full"></div>
                        </div>
                    </div>
                    @endforeach
                </div>
