...
<div class="flex flex-col md:flex-row min-h-screen bg-[#f8fafc]">
    ...
    <div class="w-full md:w-72 bg-[#003366] text-white p-8 shadow-2xl z-10 flex flex-col">
        ...
        <div class="mb-12 px-2">
            <div class="flex items-center">
                <a href="{{ url('/') }}" class="flex items-center">
                    <img src="{{ asset('public/assets/images/logo.jpeg') }}" alt="SE Global Logistics" class="h-12 md:h-16 w-auto object-contain rounded-lg">
                </a>
            </div>
        ...
        <nav class="space-y-1.5 flex-grow">
            ...
            <a href="{{ route('admin.dashboard') }}" class="flex items-center space-x-3 p-3.5 bg-gradient-to-r from-[#22C55E] to-[#16a34a] rounded-xl font-bold shadow-lg shadow-[#22C55E]/20 group transition-all">
            ...
            <a href="{{ route('admin.shipments.index') }}" class="flex items-center space-x-3 p-3.5 hover:bg-white/5 rounded-xl transition-all text-slate-300 hover:text-white group">
            ...
            <a href="{{ route('admin.customers.index') }}" class="flex items-center space-x-3 p-3.5 hover:bg-white/5 rounded-xl transition-all text-slate-300 hover:text-white group">
    ...
    <div class="flex-1 p-6 md:p-12 overflow-y-auto">
        ...
        <div class="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <a href="{{ route('admin.shipments.create') }}" class="group relative inline-flex items-center justify-center">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
            <div class="bg-white p-8 rounded-[2.5rem] shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] border border-slate-100/50 relative overflow-hidden group">
                ...
                <h4 class="text-3xl font-black text-[#003366]">{{ $stats['total'] }}</h4>
            </div>
            ...
            <div class="bg-white p-8 rounded-[2.5rem] shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] border border-slate-100/50 relative overflow-hidden group">
                ...
                <h4 class="text-3xl font-black text-[#003366]">{{ $stats['transit'] }}</h4>
            </div>
            ...
            <div class="bg-white p-8 rounded-[2.5rem] shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] border border-slate-100/50 relative overflow-hidden group">
                ...
                <h4 class="text-3xl font-black text-[#003366]">{{ $stats['delivered'] }}</h4>
            </div>
            ...
            <div class="bg-white p-8 rounded-[2.5rem] shadow-[0_10px_40px_-15px_rgba(0,0,0,0.05)] border border-slate-100/50 relative overflow-hidden group">
                ...
                <h4 class="text-3xl font-black text-[#003366]">{{ $stats['pending'] }}</h4>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[3rem] shadow-[0_20px_60px_-20px_rgba(0,0,0,0.03)] border border-slate-100 overflow-hidden">
            <div class="p-10 flex justify-between items-center border-b border-slate-50">
                ...
                <a href="{{ route('admin.shipments.index') }}" class="text-xs font-bold text-[#22C55E] bg-emerald-50 px-5 py-2.5 rounded-xl hover:bg-[#22C55E] hover:text-white transition-all">View All</a>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr class="group transition-all duration-300">
                            <td class="bg-slate-50 group-hover:bg-white group-hover:shadow-xl px-6 py-5 rounded-l-2xl border-y border-l border-transparent group-hover:border-slate-100">
                                <span class="font-mono text-xs font-bold text-[#003366] bg-white px-3 py-1.5 rounded-lg shadow-sm border border-slate-100 group-hover:border-[#22C55E]/30">{{ $shipment->tracking_number }}</span>
                            </td>
                            <td class="bg-slate-50 group-hover:bg-white group-hover:shadow-xl px-6 py-5 border-y border-transparent group-hover:border-slate-100">
                                ...
                                <p class="text-sm font-bold text-[#003366]">{{ $shipment->client_name }}</p>
                            </td>
                            <td class="bg-slate-50 group-hover:bg-white group-hover:shadow-xl px-6 py-5 border-y border-transparent group-hover:border-slate-100">
                            ...
                            <td class="bg-slate-50 group-hover:bg-white group-hover:shadow-xl px-6 py-5 border-y border-transparent group-hover:border-slate-100">
                                ...
                                @else
                                    <span class="bg-amber-50 text-amber-500 px-3 py-1 rounded-full font-black text-[9px] uppercase tracking-wider inline-flex items-center">
                                        <span class="w-1.5 h-1.5 bg-amber-500 rounded-full mr-2"></span> {{ $shipment->status }}
                                    </span>
                                @endif
                            </td>
                            <td class="bg-slate-50 group-hover:bg-white group-hover:shadow-xl px-6 py-5 rounded-r-2xl border-y border-r border-transparent group-hover:border-slate-100 text-center">
                                <a href="{{ route('admin.shipments.edit', $shipment->id) }}" class="inline-flex p-2.5 bg-white text-slate-400 hover:text-[#22C55E] hover:shadow-md rounded-xl transition-all border border-slate-200">
